// src/components/ChaosCanvas.jsx
...
/**
 * Canvas com pêndulo de 3 barras e trilha.
 * Props:
 *  - running: boolean        → anima se true
 *  - speedMul: number        → 1.0 .. ~2.2
 *  - onClickIdle?: () => void
 *
 * Observação:
 *  - Com forwardRef, o <canvas> fica acessível em App via ref.
 */
const ChaosCanvas = forwardRef(function ChaosCanvas(
  { running = false, speedMul = 1, onClickIdle },
  ref
) {
  const canvasRef = useRef(null);
  const ctxRef = useRef(null);

  // Disponibiliza o <canvas> para quem passou o ref
  useEffect(() => {
    if (!ref) return;
  ...
  const pRef = useRef({
    ...
    s01: null, s12: null, s23: null,
  });

  // Tamanho responsivo
  const { w, h, dpr } = useCanvasSize();

  // Redimensiona e prepara o contexto
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d", { alpha: false });

    // Ajustes para nitidez
  ...
  useAnimationFrame(({ dt }) => {
    ...
    if (running) {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        for (let j = 0; j < 8; j++) {
          s01.satisfy(); s12.satisfy(); s23.satisfy();
        }
      }
